...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchDesignCourses(): Promise<Course[]> {
        try {
            ...
            return courses.filter(course => course.topic === 'Design');
        } catch (error) {
    ...
    async function initializeDesignPage(): Promise<void> {
        if (!courseContainer) {
            console.error('Course container not found!');
            return;
        }

        const designCourses = await fetchDesignCourses();
        
        if (designCourses.length === 0) {
            courseContainer.innerHTML = '<p>No design courses available at the moment.</p>';
            return;
        }

        courseContainer.innerHTML = designCourses
        ...
        document.querySelectorAll('.view-details-button').forEach(button => {
            button.addEventListener('click', () => {
                ...
                const course = designCourses.find(c => c.id === courseId);
                if (course) {
