...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchMarketingCourses(): Promise<Course[]> {
        try {
            ...
            return courses.filter(course => course.topic === 'Marketing');
        } catch (error) {
    ...
    async function initializeMarketingPage(): Promise<void> {
        if (!courseContainer) {
            console.error('Course container not found!');
            return;
        }

        const marketingCourses = await fetchMarketingCourses();
        
        if (marketingCourses.length === 0) {
            courseContainer.innerHTML = '<p>No marketing courses available at the moment.</p>';
            return;
        }

        courseContainer.innerHTML = marketingCourses
        ...
        document.querySelectorAll('.view-details-button').forEach(button => {
            button.addEventListener('click', () => {
                ...
                const course = marketingCourses.find(c => c.id === courseId);
                if (course) {
